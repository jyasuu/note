這題問得非常好，**Iceberg「能寫 ≠ 寫得好」**，實務上踩雷的案例很多  
下面我用**工程實戰導向**，把 **寫入 Iceberg 常見 anti‑patterns / 陷阱** 分成幾大類，每一類都會講：

*   為什麼會出問題
*   具體壞例子
*   正確做法 / 心法

***

# 全覽地圖（先看這個）

| 類型               | Anti‑pattern        |
| ---------------- | ------------------- |
| 檔案層              | 小檔案爆炸               |
| Partition        | 過度 / 錯誤 partition   |
| Commit           | 每筆 / 高頻 commit      |
| Streaming        | micro‑batch misuse  |
| Concurrent write | 多 writer 沒協調        |
| Schema           | 隨意 schema evolution |
| Delete / Update  | row-level 操作濫用      |
| Metadata         | metadata 成長失控       |
| Catalog          | 用錯 catalog          |
| Operational      | 沒做 maintenance      |

***

## 1️⃣ **Small Files Anti‑Pattern（最常見、最致命）**

### 壞現象

*   每次寫入產生大量 <10MB 的 Parquet
*   Query 慢、planning time 長、metadata 爆炸

### 為什麼？

Iceberg **允許 small files，但不會幫你消滅它**

寫入模型是：

> 「append data files → commit snapshot」

❌ 它不會自動 merge

### 壞例子

```text
每分鐘一個 Spark job
→ 每次寫 200 個 5MB 檔
→ 一天 288,000 個檔案
```

### 正確做法 ✅

*   寫入目標檔案大小：
    *   **512MB – 1GB**
*   控制 Writer：
    *   Spark：`write.target-file-size-bytes`
*   **定期執行：**
    ```sql
    CALL system.rewrite_data_files(...)
    ```

📌 **經驗法則**：

> 「Iceberg 能承受 small files，但你不能忽視它們」

***

## 2️⃣ **Over‑Partitioning（老 Hive 思維）**

### 壞現象

```sql
PARTITIONED BY (year, month, day, hour, user_id)
```

### 為什麼是陷阱？

*   Iceberg **不是 Hive**
*   Partition ≠ 目錄剪裁
*   每個 partition 都增加 metadata cost

### Iceberg 的真強項

*   Hidden partitioning
*   Manifest‑based pruning
*   File‑level stats

### Recommended ✅

```sql
PARTITIONED BY (days(event_time))
```

或甚至：

```sql
NO PARTITION
```

📌 **反直覺真相**

> Iceberg 中「太多 partition」比「太少 partition」更糟

***

## 3️⃣ **High‑Frequency Commit（每筆就 commit）**

### 壞現象

*   每秒 commit 一次
*   Snapshot 成長超快
*   Catalog 熱點、寫入衝突

### 為什麼？

Iceberg commit 是：

    CAS compare-and-swap
    → metadata.json
    → catalog update

不是免費操作。

### 壞例子

*   REST API 每筆資料都觸發寫入
*   Streaming engine batch size = 1

### 正確做法 ✅

*   Batch writes
*   Streaming：
    *   micro‑batch > 100k rows
    *   或基於時間（1–5 min）
*   控制 snapshot 數量

***

## 4️⃣ **Streaming = 無限 Append（但沒觀念）**

### 壞現象

*   Kafka → Iceberg 直接寫
*   無 compaction
*   一週後 query 幾乎不能跑

### Iceberg Streaming 真相

Iceberg ≠ Streaming Store  
它是 **streaming‑friendly table format**

### 正確心法 ✅

Streaming pipeline = **三段式**

1.  ingest (append)
2.  **rewrite\_data\_files**
3.  expire\_snapshots

```text
Streaming writes
↓
Small files
↓
Scheduled compaction
```

> ❌ 沒 compaction = 設計錯誤

***

## 5️⃣ **多 Writer 沒規範（Concurrency Hell）**

### 壞現象

*   Spark job A 寫
*   Flink job B 寫
*   成功率低、commit fail

### 為什麼？

*   Iceberg 用 optimistic concurrency
*   同 table 同時寫 → 高機率衝突

### Anti‑pattern

    ETL job
    + Ad‑hoc notebook
    + Streaming job
    → 同一張 table

### 正確做法 ✅

*   **一張 table 一種 writer 類型**
*   或：
    *   append‑only table
    *   merge table 分離
*   使用 REST / managed catalog

***

## 6️⃣ **濫用 UPDATE / DELETE / MERGE**

### 壞現象

```sql
DELETE FROM table WHERE date < '2023-01-01';
```

### 為什麼危險？

Iceberg 的 row‑level ops：

*   equality delete file
*   position delete file

讀取時要：

    data file + delete files

會造成：

*   read amplification
*   metadata 複雜度上升

### 正確策略 ✅

*   能 **rewrite data files** 就 rewrite
*   大範圍修改：
    *   CTAS
    *   INSERT OVERWRITE（Iceberg 狀態）

📌 原則：

> Delete 是手術刀，不是推土機

***

## 7️⃣ **Schema Evolution 當成免費**

### 壞現象

*   任意 rename column
*   type 頻繁變更
*   同名不同意義

### Iceberg 是 schema‑on‑read，不代表：

*   consumer 免改
*   downstream 不炸

### 正確做法 ✅

*   schema change = API contract
*   rename ≠ drop + add（語意不同）
*   versioned rollout

***

## 8️⃣ **Metadata 無限成長（忘了做 housekeeping）**

### 常被忽略的大坑

*   snapshots
*   manifests
*   manifest lists

### 後果

*   plan time 超長
*   Driver OOM
*   Catalog latency

### 必做 ✅

```sql
CALL system.expire_snapshots(...)
CALL system.rewrite_manifests(...)
CALL system.remove_orphan_files(...)
```

📌 **Iceberg ≠ zero‑ops**

***

## 9️⃣ **選錯 Catalog，然後怪 Iceberg**

### 經典誤會

> 「Iceberg 寫入常 fail，不穩定」

實際上：

*   用 Hadoop catalog
*   S3 eventual consistency
*   多 writer

### 正確做法 ✅

*   POC：Hadoop catalog ✅
*   Production：
    *   REST
    *   Glue
    *   Managed catalog

Catalog 選錯 = 架構就錯一半

***

## 10️⃣ **把 Iceberg 當 OLTP**

### 壞期待

*   sub‑second write latency
*   high QPS random writes

### 現實

Iceberg 是：
✅ analytical table format  
❌ transactional DB

### 正確定位

*   ingestion layer ✅
*   analytical store ✅
*   serving store ❌

***

# 一句話總結 Anti‑Patterns

> **Iceberg 的寫入問題，90% 不是 Iceberg 的問題，而是「拿 OLTP / Hive mindset 強套在 Data Lake」**

***

如果你願意，我可以下一步幫你：

*   ✅ 用 **checklist** 幫你 review 你現在的 Iceberg pipeline
*   ✅ 針對 **Spark / Flink / Trino** 各別列出寫入陷阱
*   ✅ 幫你設計 **Streaming → Iceberg 的正確模板**
*   ✅ Debug 現在你遇到的寫入/效能問題

你目前是 **batch 還是 streaming 寫入 Iceberg？用的是哪個 engine？**

```toml
[package]
name = "iceberg-cli"
version = "0.1.0"
edition = "2024"

[[bin]]
name = "iceberg-cli"
path = "src/main.rs"
 
[dependencies]
# Iceberg core + REST catalog
iceberg              = { version = "0.9.0" }
iceberg-catalog-rest = "0.9.0"
 
# Async runtime
tokio   = { version = "1",  features = ["full"] }
futures = "0.3"
 
# CLI argument parsing
clap = { version = "4", features = ["derive","env"] }
 
# Pretty output
comfy-table = "7"
serde_json  = "1"
 
# Arrow / Parquet
arrow-array  = "57.1"
arrow-schema = "57.1"
arrow-cast   = "57.1"
parquet      = "57.1"
 
# Error handling
anyhow = "1"
```

```rs
//! iceberg-cli — minimal CLI to list, query and write Iceberg tables
//! via a REST catalog backed by S3.
//!
//! Usage examples
//! ──────────────
//!   # List namespaces
//!   iceberg-cli --uri http://localhost:8181 list-namespaces
//!
//!   # List tables inside a namespace
//!   iceberg-cli --uri http://localhost:8181 list-tables --namespace my_db
//!
//!   # Show table schema / metadata
//!   iceberg-cli --uri http://localhost:8181 describe --table my_db.products
//!
//!   # Scan / query a table (optional column projection + limit)
//!   iceberg-cli --uri http://localhost:8181 scan \
//!       --table my_db.products \
//!       --columns id,name,price \
//!       --limit 20
//!
//!   # Write rows (JSON array) into a table via fast-append
//!   iceberg-cli --uri http://localhost:8181 write \
//!       --table my_db.products \
//!       --json '[{"id":1,"name":"Widget","price":9.99}]'
//!
//!   # Create a namespace
//!   iceberg-cli --uri http://localhost:8181 create-namespace --namespace my_db
//!
//!   # Create a table  (id LONG, name STRING, price DOUBLE)
//!   iceberg-cli --uri http://localhost:8181 create-table \
//!       --table my_db.products \
//!       --schema "id:long,name:string,price:double"

use std::{collections::HashMap, sync::Arc};

use anyhow::{Context, Result, anyhow, bail};
use arrow_array::{ArrayRef, Float64Array, Int64Array, RecordBatch, StringArray};
use arrow_schema::{DataType, Field, Schema as ArrowSchema};
use clap::{Parser, Subcommand};
use comfy_table::{Table, presets::UTF8_FULL};
use futures::TryStreamExt;
use iceberg::{
    Catalog, CatalogBuilder, NamespaceIdent, TableCreation, TableIdent,
    spec::{
        DataFileFormat, NestedField, PartitionKey, PartitionSpec, PrimitiveType, Schema, Struct,
        Type,
    },
    transaction::{ApplyTransactionAction, Transaction},
    writer::{
        IcebergWriter, IcebergWriterBuilder,
        base_writer::data_file_writer::DataFileWriterBuilder,
        file_writer::{
            ParquetWriterBuilder,
            location_generator::{DefaultFileNameGenerator, DefaultLocationGenerator},
        },
    },
};
use iceberg_catalog_rest::{REST_CATALOG_PROP_URI, RestCatalogBuilder};
use parquet::file::properties::WriterProperties;

// ─── CLI definition ──────────────────────────────────────────────────────────

#[derive(Parser)]
#[command(
    name = "iceberg-cli",
    about = "Minimal Iceberg CLI (REST catalog + S3)",
    version
)]
struct Cli {
    /// REST catalog URI (e.g. http://localhost:8181)
    #[arg(long, env = "ICEBERG_URI", default_value = "http://localhost:8181")]
    uri: String,

    /// AWS region for S3 (or set AWS_DEFAULT_REGION)
    #[arg(long, env = "AWS_DEFAULT_REGION", default_value = "us-east-1")]
    region: String,

    /// Optional catalog name
    #[arg(long, default_value = "default")]
    catalog: String,

    #[command(subcommand)]
    cmd: Commands,
}

#[derive(Subcommand)]
enum Commands {
    /// List namespaces in the catalog
    ListNamespaces,

    /// List tables inside a namespace
    ListTables {
        /// Namespace (e.g. my_db)
        #[arg(long)]
        namespace: String,
    },

    /// Show table schema and metadata
    Describe {
        /// Fully-qualified table: namespace.table
        #[arg(long)]
        table: String,
    },

    /// Scan / read a table into the terminal
    Scan {
        /// Fully-qualified table: namespace.table
        #[arg(long)]
        table: String,

        /// Comma-separated column names to project (default: all)
        #[arg(long)]
        columns: Option<String>,

        /// Max rows to display  (default: 50)
        #[arg(long, default_value = "50")]
        limit: usize,
    },

    /// Write rows (JSON array) into a table via fast-append
    Write {
        /// Fully-qualified table: namespace.table
        #[arg(long)]
        table: String,

        /// JSON array of objects, e.g. '[{"id":1,"name":"x","price":1.5}]'
        #[arg(long)]
        json: String,

        /// Schema: comma-separated "col:type" pairs.
        /// Supported types: long, string, double, boolean, int, float, date, timestamp
        /// Example: "id:long,name:string,price:double"
        #[arg(long)]
        schema: String,
    },

    /// Create a new namespace
    CreateNamespace {
        #[arg(long)]
        namespace: String,
    },

    /// Create a new table with a simple schema
    CreateTable {
        /// Fully-qualified table: namespace.table
        #[arg(long)]
        table: String,

        /// Schema: comma-separated "col:type" pairs.
        /// Supported types: long, string, double, boolean, int, float, date, timestamp
        /// Example: "id:long,name:string,price:double"
        #[arg(long)]
        schema: String,
    },
}

// ─── Entry point ─────────────────────────────────────────────────────────────

#[tokio::main]
async fn main() -> Result<()> {
    let cli = Cli::parse();

    let catalog = connect(&cli.uri, &cli.catalog, &cli.region).await?;

    match &cli.cmd {
        Commands::ListNamespaces => cmd_list_namespaces(&catalog).await?,
        Commands::ListTables { namespace } => cmd_list_tables(&catalog, namespace).await?,
        Commands::Describe { table } => cmd_describe(&catalog, table).await?,
        Commands::Scan {
            table,
            columns,
            limit,
        } => cmd_scan(&catalog, table, columns.as_deref(), *limit).await?,
        Commands::Write {
            table,
            json,
            schema,
        } => cmd_write(&catalog, table, json, schema).await?,
        Commands::CreateNamespace { namespace } => {
            cmd_create_namespace(&catalog, namespace).await?
        }
        Commands::CreateTable { table, schema } => {
            cmd_create_table(&catalog, table, schema).await?
        }
    }

    Ok(())
}

// ─── Catalog connection ───────────────────────────────────────────────────────

async fn connect(uri: &str, name: &str, region: &str) -> Result<impl Catalog> {
    let catalog = RestCatalogBuilder::default()
        .load(
            name,
            HashMap::from([
                (REST_CATALOG_PROP_URI.to_string(), uri.to_string()),
                ("s3.region".to_string(), region.to_string()),
            ]),
        )
        .await
        .with_context(|| format!("Failed to connect to REST catalog at {uri}"))?;
    Ok(catalog)
}

// ─── Commands ─────────────────────────────────────────────────────────────────

async fn cmd_list_namespaces(catalog: &impl Catalog) -> Result<()> {
    let namespaces = catalog
        .list_namespaces(None)
        .await
        .context("list_namespaces")?;

    if namespaces.is_empty() {
        println!("(no namespaces)");
        return Ok(());
    }

    let mut tbl = Table::new();
    tbl.load_preset(UTF8_FULL).set_header(vec!["Namespace"]);
    for ns in &namespaces {
        tbl.add_row(vec![ns.to_url_string()]);
    }
    println!("{tbl}");
    Ok(())
}

async fn cmd_list_tables(catalog: &impl Catalog, namespace: &str) -> Result<()> {
    let ns = parse_namespace(namespace)?;
    let tables = catalog
        .list_tables(&ns)
        .await
        .with_context(|| format!("list_tables({namespace})"))?;

    if tables.is_empty() {
        println!("(no tables in namespace '{namespace}')");
        return Ok(());
    }

    let mut tbl = Table::new();
    tbl.load_preset(UTF8_FULL)
        .set_header(vec!["Namespace", "Table"]);
    for t in &tables {
        tbl.add_row(vec![t.namespace().to_url_string(), t.name().to_string()]);
    }
    println!("{tbl}");
    Ok(())
}

async fn cmd_describe(catalog: &impl Catalog, table_str: &str) -> Result<()> {
    let ident = parse_table(table_str)?;
    let table = catalog
        .load_table(&ident)
        .await
        .with_context(|| format!("load_table({table_str})"))?;
    let meta = table.metadata();

    println!("━━━  Table: {table_str}  ━━━");
    println!("  Format version : {:?}", meta.format_version());
    println!("  Location       : {}", meta.location());

    if let Some(snap) = meta.current_snapshot() {
        println!("  Snapshot ID    : {}", snap.snapshot_id());
        println!("  Manifest list  : {}", snap.manifest_list());
    } else {
        println!("  Snapshot       : (none — empty table)");
    }

    println!("\nSchema:");
    let mut tbl = Table::new();
    tbl.load_preset(UTF8_FULL)
        .set_header(vec!["Field ID", "Name", "Type", "Required"]);
    for field in meta.current_schema().as_struct().fields() {
        tbl.add_row(vec![
            field.id.to_string(),
            field.name.clone(),
            format!("{:?}", field.field_type),
            field.required.to_string(),
        ]);
    }
    println!("{tbl}");

    let props = meta.properties();
    if !props.is_empty() {
        println!("\nProperties:");
        let mut ptbl = Table::new();
        ptbl.load_preset(UTF8_FULL).set_header(vec!["Key", "Value"]);
        for (k, v) in props {
            ptbl.add_row(vec![k, v]);
        }
        println!("{ptbl}");
    }

    Ok(())
}

async fn cmd_scan(
    catalog: &impl Catalog,
    table_str: &str,
    columns: Option<&str>,
    limit: usize,
) -> Result<()> {
    let ident = parse_table(table_str)?;
    let table = catalog
        .load_table(&ident)
        .await
        .with_context(|| format!("load_table({table_str})"))?;

    let mut scan_builder = table.scan();

    if let Some(cols) = columns {
        let col_list: Vec<&str> = cols.split(',').map(str::trim).collect();
        scan_builder = scan_builder.select(col_list);
    }

    let stream = scan_builder
        .build()
        .context("build scan")?
        .to_arrow()
        .await
        .context("to_arrow")?;

    let batches: Vec<RecordBatch> = stream.try_collect().await.context("collect batches")?;

    if batches.is_empty() {
        println!("(no data)");
        return Ok(());
    }

    let schema = batches[0].schema();
    let headers: Vec<&str> = schema.fields().iter().map(|f| f.name().as_str()).collect();

    let mut tbl = Table::new();
    tbl.load_preset(UTF8_FULL).set_header(headers);

    let mut row_count = 0usize;
    'outer: for batch in &batches {
        for row in 0..batch.num_rows() {
            if row_count >= limit {
                break 'outer;
            }
            let cells: Vec<String> = (0..batch.num_columns())
                .map(|col| arrow_value_to_string(batch.column(col), row))
                .collect();
            tbl.add_row(cells);
            row_count += 1;
        }
    }
    println!("{tbl}");
    println!("({row_count} rows shown, limit={limit})");
    Ok(())
}

async fn cmd_write(
    catalog: &impl Catalog,
    table_str: &str,
    json: &str,
    schema_str: &str,
) -> Result<()> {
    let ident = parse_table(table_str)?;
    let table = catalog
        .load_table(&ident)
        .await
        .with_context(|| format!("load_table({table_str})"))?;

    let rows: Vec<serde_json::Value> =
        serde_json::from_str(json).context("parse --json argument")?;
    if rows.is_empty() {
        println!("Nothing to write (empty JSON array).");
        return Ok(());
    }

    let batch = json_rows_to_record_batch(&rows, table.metadata().current_schema())
        .context("convert JSON -> RecordBatch")?;

    let location_gen = DefaultLocationGenerator::new(table.metadata().clone())
        .context("DefaultLocationGenerator")?;
    let file_name_gen =
        DefaultFileNameGenerator::new("data".to_string(), None, DataFileFormat::Parquet);

    let parquet_builder = ParquetWriterBuilder::new(
        WriterProperties::builder().build(),
        table.metadata().current_schema().clone(),
    );

    let rolling_file_writer_builder =
        iceberg::writer::file_writer::rolling_writer::RollingFileWriterBuilder::new(
            parquet_builder,
            536_870_912usize,
            table.file_io().clone(),
            location_gen,
            file_name_gen,
        );

    let schema = parse_schema(schema_str).context("parse --schema")?;
    let schema = Arc::new(schema);
    let mut partition_key: Option<PartitionKey> = None;
    if let Some(field) = schema.name_by_field_id(1) {
        let spec = PartitionSpec::builder(schema.clone())
            .add_partition_field(field, field, iceberg::spec::Transform::Identity)?
            .build()?;
        let partition_data =
            Struct::from_iter(vec![Some(iceberg::spec::Literal::string("2024-04-23"))]);
        partition_key = Some(PartitionKey::new(
            spec.clone(),
            schema.clone(),
            partition_data,
        ));
    }

    let mut writer = DataFileWriterBuilder::new(rolling_file_writer_builder)
        .build(partition_key)
        .await
        .context("build writer")?;

    writer.write(batch).await.context("write batch")?;
    let data_files = writer.close().await.context("close writer")?;
    println!("Wrote {} Parquet file(s).", data_files.len());

    let tx = Transaction::new(&table);
    let action = tx.fast_append().add_data_files(data_files);
    let tx = action.apply(tx).context("apply transaction")?;
    let updated = tx.commit(catalog).await.context("commit transaction")?;

    println!(
        "Committed. New snapshot ID: {:?}",
        updated
            .metadata()
            .current_snapshot()
            .map(|s| s.snapshot_id())
    );
    Ok(())
}

async fn cmd_create_namespace(catalog: &impl Catalog, namespace: &str) -> Result<()> {
    let ns = parse_namespace(namespace)?;
    catalog
        .create_namespace(&ns, HashMap::new())
        .await
        .with_context(|| format!("create_namespace({namespace})"))?;
    println!("Created namespace '{namespace}'.");
    Ok(())
}

async fn cmd_create_table(catalog: &impl Catalog, table_str: &str, schema_str: &str) -> Result<()> {
    let ident = parse_table(table_str)?;
    let schema = parse_schema(schema_str).context("parse --schema")?;

    let creation = TableCreation::builder()
        .name(ident.name().to_string())
        .schema(schema)
        .build();

    catalog
        .create_table(ident.namespace(), creation)
        .await
        .with_context(|| format!("create_table({table_str})"))?;

    println!("Created table '{table_str}'.");
    Ok(())
}

// ─── Helpers ─────────────────────────────────────────────────────────────────

fn parse_namespace(s: &str) -> Result<NamespaceIdent> {
    NamespaceIdent::from_strs(s.split('.').collect::<Vec<_>>())
        .map_err(|e| anyhow!("Invalid namespace '{s}': {e}"))
}

fn parse_table(s: &str) -> Result<TableIdent> {
    let parts: Vec<&str> = s.rsplitn(2, '.').collect();
    match parts.as_slice() {
        [name, namespace] => {
            let ns = parse_namespace(namespace)?;
            Ok(TableIdent::new(ns, name.to_string()))
        }
        _ => bail!("Expected 'namespace.table', got '{s}'"),
    }
}

fn parse_schema(spec: &str) -> Result<Schema> {
    let mut fields = Vec::new();
    for (idx, part) in spec.split(',').enumerate() {
        let kv: Vec<&str> = part.trim().splitn(2, ':').collect();
        if kv.len() != 2 {
            bail!("Bad field spec '{part}' — expected 'name:type'");
        }
        let name = kv[0].trim();
        let ptype = match kv[1].trim().to_lowercase().as_str() {
            "long" | "int64" => PrimitiveType::Long,
            "int" | "int32" => PrimitiveType::Int,
            "string" | "str" => PrimitiveType::String,
            "double" | "float64" => PrimitiveType::Double,
            "float" | "float32" => PrimitiveType::Float,
            "boolean" | "bool" => PrimitiveType::Boolean,
            "date" => PrimitiveType::Date,
            "timestamp" => PrimitiveType::Timestamp,
            other => bail!("Unknown type '{other}'"),
        };
        let field_id = (idx + 1) as i32;
        fields.push(NestedField::optional(field_id, name, Type::Primitive(ptype)).into());
    }

    Schema::builder()
        .with_fields(fields)
        .with_schema_id(1)
        .build()
        .context("build schema")
}

fn arrow_value_to_string(array: &ArrayRef, row: usize) -> String {
    use arrow_array::Array;
    if array.is_null(row) {
        return "NULL".to_string();
    }
    macro_rules! downcast_fmt {
        ($ty:ty) => {
            if let Some(a) = array.as_any().downcast_ref::<$ty>() {
                return a.value(row).to_string();
            }
        };
    }
    downcast_fmt!(arrow_array::Int8Array);
    downcast_fmt!(arrow_array::Int16Array);
    downcast_fmt!(arrow_array::Int32Array);
    downcast_fmt!(arrow_array::Int64Array);
    downcast_fmt!(arrow_array::UInt8Array);
    downcast_fmt!(arrow_array::UInt16Array);
    downcast_fmt!(arrow_array::UInt32Array);
    downcast_fmt!(arrow_array::UInt64Array);
    downcast_fmt!(arrow_array::Float32Array);
    downcast_fmt!(arrow_array::Float64Array);
    downcast_fmt!(arrow_array::BooleanArray);
    if let Some(a) = array.as_any().downcast_ref::<arrow_array::StringArray>() {
        return a.value(row).to_string();
    }
    if let Some(a) = array
        .as_any()
        .downcast_ref::<arrow_array::LargeStringArray>()
    {
        return a.value(row).to_string();
    }
    "(unsupported)".to_string()
}

fn json_rows_to_record_batch(rows: &[serde_json::Value], schema: &Schema) -> Result<RecordBatch> {
    let mut arrow_fields: Vec<Field> = Vec::new();
    let mut columns: Vec<ArrayRef> = Vec::new();

    for field in schema.as_struct().fields() {
        let col = field.name.as_str();
        match &*field.field_type {
            Type::Primitive(PrimitiveType::Long) | Type::Primitive(PrimitiveType::Int) => {
                let vals: Vec<Option<i64>> = rows
                    .iter()
                    .map(|r| r.get(col).and_then(|v| v.as_i64()))
                    .collect();
                arrow_fields.push(Field::new(col, DataType::Int64, true));
                columns.push(Arc::new(Int64Array::from(vals)) as ArrayRef);
            }
            Type::Primitive(PrimitiveType::String) => {
                let vals: Vec<Option<&str>> = rows
                    .iter()
                    .map(|r| r.get(col).and_then(|v| v.as_str()))
                    .collect();
                arrow_fields.push(Field::new(col, DataType::Utf8, true));
                columns.push(Arc::new(StringArray::from(vals)) as ArrayRef);
            }
            Type::Primitive(PrimitiveType::Double) | Type::Primitive(PrimitiveType::Float) => {
                let vals: Vec<Option<f64>> = rows
                    .iter()
                    .map(|r| r.get(col).and_then(|v| v.as_f64()))
                    .collect();
                arrow_fields.push(Field::new(col, DataType::Float64, true));
                columns.push(Arc::new(Float64Array::from(vals)) as ArrayRef);
            }
            _ => {
                let vals: Vec<Option<String>> = rows
                    .iter()
                    .map(|r| r.get(col).map(|v| v.to_string()))
                    .collect();
                arrow_fields.push(Field::new(col, DataType::Utf8, true));
                let arr: StringArray = vals.iter().map(|v| v.as_deref()).collect();
                columns.push(Arc::new(arr) as ArrayRef);
            }
        }
    }

    let arrow_schema = Arc::new(ArrowSchema::new(arrow_fields));
    RecordBatch::try_new(arrow_schema, columns).context("build RecordBatch")
}

```
