...
以下用「系統程式開發的角度」帶你拆解這段程式碼，特別聚焦在：

1. **Cursor 為什麼在 parse_frame 裡用？**
2. **Cursor 在 write_decimal 裡為什麼也用？**
3. 整個 Connection 的 I/O 行為、資料流、解析流程
4. BytesMut + Cursor 的組合如何打造高效 protocol parser
5. 你日後要自己寫 binary framing / 自訂協定時該怎麼套用這個模式

---

# 🔥 核心觀念：Cursor 是「memory 版的檔案指標」

在 network protocol 解析中你會一直需要：

* 查看 buffer 是否有足夠 bytes（但不消耗它）
* 真正解析 frame（並消耗 bytes）

Cursor 可以讓你：

* 用 `position()` 知道目前解析到哪裡
* 用 `set_position()` 重置游標
* **不會修改底層 BytesMut**
* 又能使用 `Buf` trait 的便利方法（bytes crate 提供）
...
```
TCPStream <---- BufWriter ----> write_frame()
TCPStream <---- buffer(BytesMut) ----> read_frame()
                               |
                               +-- parse_frame(Cursor)
```
...
```
socket → BytesMut → Cursor → Frame
```

寫的路徑：

```
Frame → BufWriter → socket
```

---

# 🧩 為什麼 parse_frame 用 Cursor？
...
* 我想要「從 BytesMut 讀資料」
* 但 **不想真的把 BytesMut 的資料彈掉（消耗掉）**
* 並希望用 Cursor 的「游標 position」功能來推估 frame 長度

Cursor 的行為是：

* 只動它自己的指標，不動 self.buffer
* parse/check 完後你可以決定要不要 `advance()` buffer

### 🎯 parse_frame 的兩階段解析流程

這是 protocol parser 最經典的寫法：

1. **第一階段：Frame::check(&mut buf)**

   * 目的：看看現在 buffer 內是否已收到完整 frame
   * 優點：不建立任何額外資料結構，輕量快速
   * 如果不夠 → return Ok(None)
   * 如果夠 → Cursor.position() 就是 frame 長度

2. **第二階段：真正解析 Frame::parse(&mut buf)**

   * 重設 buf position 為 0
   * 做實體解析 → 建資料結構（Vec、String、Bulk 等）
...
把這 frame 從 BytesMut 彈掉，只留下未解析的資料。
...
* 你沒辦法用 position 追蹤 frame 長度
* 你沒辦法用 set_position(0) 重新解析同一段 bytes
* 你不能用 bytes::Buf 的便利方法（get_u8、get_u64 等）
* 如果直接操作 BytesMut，會造成資料被提前消耗

Cursor 就是「可 seek 的 buffer reader」。

---

# 📦 BytesMut：高效、可成長的 byte buffer
...
* 只移動內部 offset，不需要 copy
* 如果 buffer 太小，後續 TCP read 時 ByteMut 會自動 grow

Cursor 沒有修改 buffer，因此 advance 才能正確丟掉封包。

---

# ✏ write_decimal 裡為什麼用 Cursor？

寫一個 decimal 的 Redis 數字（例如 `:1234\r\n`）需要把 u64 → 十進位字串。

方法是：

1. 准備一塊 stack buffer（20 bytes）
2. 用 Cursor 包起來 → 讓 write! 可以把文字寫進去
...
* 保持一個 position（從 0 開始）
* 每寫一個字元就往前走
* 最後用 `buf.position()` 得到寫入長度

再把 buf 的有效片段送到 socket：
...
這比使用 `val.to_string()` 更快：

* 不會做 heap allocation
* 不會有 String / Vec 的脫離 stack 的成本
* 效能好很多（protocol 層常用）

---

# 🧠 宏觀理解：這是典型的 TCP protocol framing pattern

對於 stream-based protocol，你永遠遇到以下問題：

### ❗ TCP 是 stream，不保證封包邊界

一個 Redis Command 可能：

* 分成 5 次 read 才收完
* 或一次 read 收兩個 command

所以你需要：

* **累積**（BytesMut）
* **檢查 frame 完整性**（Frame::check）
* **解析後彈掉資料**（advance）

Cursor 讓你能在 BytesMut 上：
...
這是所有 protocol parser 的經典寫法，不只 Redis，
HTTP、PostgreSQL protocol、Kafka protocol、WebSocket framing 都是同樣的模式。

---

# 🏗 你自己要寫 framing 時可以直接照這個 pattern 套用

假設你要解析自訂 binary 協定：

```
| magic(1B) | len(4B) | payload(len bytes) |
```

你可以：

1. read_frame 裡用 BytesMut 累積 TCP 資料
2. parse_frame 裡：
...
// frame完整，得到 buf.position() = 5 + len

buf.set_position(0);
let frame = MyFrame::parse(&mut buf)?;

// advance(len + 5)
self.buffer.advance(frame_len);
```

這與 mini-redis 的模式一模一樣。

---

# 如果你想，我可以接著教你下面其中一個：

1. **畫圖解釋 BytesMut 與 Cursor 的記憶體佈局**
2. **教你如何自己寫一個 Redis frame parser（從 0 開始）**
3. **教你如何寫一個 binary protocol parser（ELF style）**
4. **講解 async read_buf / BufWriter 的底層行為**
5. **深入解析 mini-redis 的 Frame::check 與 Frame::parse**

你想往哪邊學？


短答：
**不會有 CPU busy loop 問題**，因為 loop 裡唯一會阻塞的點是：

```rust
self.stream.read_buf(&mut self.buffer).await
```

這是 *async-await I/O wait*，不是 CPU spin。
...
### **關鍵：`read_buf().await` 會讓 task 進入 Poll::Pending，不會佔 CPU**

Tokio 的 `AsyncReadExt::read_buf`：

* 沒資料進來時 → **立即 return Poll::Pending**
* 讓當前 task 暫停
* OS/epoll/kqueue 通知資料可讀時才喚醒 task
...
**parse_frame → 不完整 → read_buf.await → task sleep → 資料來 → wake → parse_frame ...**

這種 loop 模式叫做：
**"async state machine with I/O blocking point"**
完全不會 busy spin。

---

# 🔥 什麼情況才會變成 CPU issue?（這段 code 不會）

你需要以下條件才會形成 CPU 100%：

### ❌ 狀況 1：loop 裡沒有 `.await`

如果 read_buf 是 blocking I/O 或 loop 沒 await → 會爆 CPU。

### ❌ 狀況 2：`read_buf` 不阻塞（例如用 memory buffer 或 fake stream）

只在測試 mock stream 時可能會。

### ❌ 狀況 3：protocol parser 永遠回 Err(Incomplete) 而沒進 await

但這段程式碼 parse 失敗後立即呼叫 `.await` → 不會 spin。

---

# 📌 parse_frame 是純 CPU parsing，但非常快
...
# 🧠 模式說明：這叫做 “read until complete frame”

這是所有 stream-based protocol 的標準寫法：

* Http/Redis/PostgreSQL/Kafka
* WebSocket frame parsing
* Binary protocol framing

通則：
*在 while loop 裡 parse，不夠 → await read → parse，不夠 → await read*

這是最常見、最安全、也是 tokio 源碼本身使用的模式。
...
loop:
    parse_frame?
        Yes → return
        No → read_buf.await  ←--- I/O wait, not CPU
            0? → closed
            data → continue
```

`read_buf.await` 是 CPU 釋放點。

---

# 🧪 想驗證？加 logs 可觀察行為：

如果你加 `println!("loop")` 會發現：

* 沒資料 → log 不刷（task sleep）
* 有資料 → log 只刷一次、解析完就回

這也是判斷 async loop 是否 busy spin 的方法。

---

# 🟢 結論：這段 code 是 **標準、正確** 的 async framing loop

不會造成 CPU spin，也不需要額外 sleep/yield。

如果你需要，我可以繼續幫你講：

* 如何判斷 async loop 是否會 busy spin（實戰模式）
* 如果 parser 變複雜，如何避免 CPU hotspot
* 這段 mini-redis 讀取模式與 tokio 官方 pattern 的比較
* 如何壓測這段 code 的 CPU/throughput
